...
class Worker:
    ...
    def excute(self):
        ...
        avg_max_diff, avg_mean_diff, this_tensor_mutation_strategy = calFitness(g)
        ...
        #TODO feedback
        #张量突变策略的反馈
        for i in range(0,len(GlobalConfig.tensor_mutation_strategies)):
            if GlobalConfig.tensor_mutation_strategies[i] == this_tensor_mutation_strategy:
                GlobalConfig.tensor_mutation_strategy_weight[i] += thisFitness - g.fitness
                if GlobalConfig.tensor_mutation_strategy_weight[i] < 1e-6:
                    GlobalConfig.tensor_mutation_strategy_weight[i] = 1e-6
        #基本算子的反馈
